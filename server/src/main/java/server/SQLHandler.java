...
public class SQLHandler {
    ...
    private static void prepareAllStatements() throws SQLException {
        psGetNickname = connection.prepareStatement("SELECT nickname FROM usersData WHERE login = ? AND password = ?;");
        psRegistration = connection.prepareStatement("INSERT INTO usersData(login, password, nickname) VALUES (? ,? ,? );");
        psChangeNick = connection.prepareStatement("UPDATE usersData SET nickname = ? WHERE nickname = ?;");


        psAddMessage = connection.prepareStatement("INSERT INTO messages (sender, receiver, text, date) VALUES (\n" +
                "(SELECT id FROM usersData WHERE nickname=?),\n" +
                "(SELECT id FROM usersData WHERE nickname=?),\n" +
                "?, ?)");


        psGetMessageForNick = connection.prepareStatement("SELECT (SELECT nickname FROM usersData Where id = sender), \n" +
                "       (SELECT nickname FROM usersData Where id = receiver),\n" +
                "       text,\n" +
                "       date \n" +
                "FROM messages \n" +
                "WHERE sender = (SELECT id FROM usersData WHERE nickname=?)\n" +
                "OR receiver = (SELECT id FROM usersData WHERE nickname=?)\n" +
                "OR receiver = (SELECT id FROM usersData WHERE nickname='null')");
